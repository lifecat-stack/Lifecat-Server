package com.ten.service.service.record;

import com.ten.entity.RecordPostDO;
import com.ten.service.service.BaseService;
import com.ten.vo.PostVO;
import com.ten.vo.RecordVO;

/**
 * list   :todo
 * get    :获取用户某条record对应的post信息
 * post   :添加新的c_record_post记录到record中
 * put    :更新c_record_post
 * delete :删除c_record_post
 *
 * @author Administrator
 */
public interface RecordPostService extends BaseService<PostVO, RecordPostDO> {
}
